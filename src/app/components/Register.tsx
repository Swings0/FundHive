...
// Error if phone is not valid
  ...
  if(existingUser) {
   ...
   setLoading(false);
    ...
    const res = await axios.post('/api/register', {fullname,username,email,password,phone})
    if (res.status === 200){
     ...
     className='w-screen lg:w-full md:w-screen fixed  lg:h-[100vh] h-screen flex flex-col justify-center items-center bg-no-repeat bg-cover bg-center bg '
      >
        <div className='w-[22rem] flex flex-col fixed justify-center items-left bg-white/40 backdrop-blur-lg py-8 px-8 rounded-md sm:shadow-md border-t-2  sm:border-2 border-white lg:py-11 sm:mb-0 mb-4'>
          ...
          <div className='w-[6rem] h-[1.5px] bg-gradient-to-r from-[#86abd4] via-[#c7d5e6] to-[#6190c7] ml-3 opacity-60'></div>
          <div className='w-[4rem] h-[1.5px] bg-gradient-to-r from-[#86abd4] via-[#c7d5e6] to-[#6190c7] ml-3 opacity-50'></div>
          <div className='w-[2rem] h-[1.5px] bg-gradient-to-r from-[#86abd4] via-[#c7d5e6] to-[#6190c7] mb-3 ml-3 opacity-40'></div> */}

            <form onSubmit={submitHandler} className='flex flex-col gap-5 py-6'>
                <div className='w-full relative'>
                <input onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setEmail(e.target.value)} className= {`${emailError && 'border-2 border-white  w-full rounded-lg py-1 px-4 bg-red-50 text-base lg:text-sm outline-none  text-red-600 text-opacity-90 placeholder-opacity-70 placeholder-red-600 focus:border-red-200' || 'border-2 border-white focus:border-blue-200  w-full rounded-lg py-1 px-4 bg-slate-50 text-base lg:text-sm  outline-none placeholder-blue-400 text-blue-900 text-opacity-90 placeholder-opacity-70'}`} type='email' placeholder={`${emailError && 'please enter email' || 'Email'}`} /> <div className={( emailError ? 'absolute top-2 right-4 text-sm text-red-600 opacity-50 z-10' :  'absolute top-2 right-4 text-sm text-blue-400 opacity-50 z-10')}><IoMdMail/></div>
                </div>

                <div className='w-full relative'>
                <input onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setFullName(e.target.value)} className= {`${fullNameError && 'border-2 border-white  w-full rounded-lg py-1 px-4 bg-red-50 text-base lg:text-sm outline-none  text-red-600 text-opacity-90 placeholder-opacity-70 placeholder-red-600 focus:border-red-200' || 'border-2 border-white focus:border-blue-200   w-full rounded-lg py-1 px-4 bg-slate-50 text-base lg:text-sm  outline-none placeholder-blue-400 text-blue-900 text-opacity-90 placeholder-opacity-70'}`} type='text' placeholder={`${emailError && 'please enter name' || 'Fullname'}`} /> <div className={( fullNameError ? 'absolute top-2 right-4 text-sm text-red-600 opacity-50 z-10' :  'absolute top-2 right-4 text-sm text-blue-400 opacity-50 z-10')}><MdDriveFileRenameOutline/></div>
                </div>

                <div className='w-full relative'>
                <input onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setUsername(e.target.value)} className= {`${usernameError && 'border-2 border-white  w-full rounded-lg py-1 px-4 bg-red-50 text-base lg:text-sm outline-none  text-red-600 text-opacity-90 placeholder-opacity-70 placeholder-red-600 focus:border-red-200' || 'border-2 border-white focus:border-blue-200  w-full rounded-lg py-1 px-4 bg-slate-50 text-base lg:text-sm  outline-none placeholder-blue-400 text-blue-900 text-opacity-90 placeholder-opacity-70'}`} type='text' placeholder={`${usernameError && 'please enter username' || 'Username'}`} /> <div className={( usernameError ? 'absolute top-2 right-4 text-sm text-red-600 opacity-50 z-10' :  'absolute top-2 right-4 text-sm text-blue-400 opacity-50 z-10')}><FaUser/></div>
                </div>

                <div className='w-full relative'>
                <input onChange={(e:React.ChangeEvent<HTMLInputElement>)=> setPassword(e.target.value)} className= {`${passwordError && 'border-2 border-white  w-full rounded-lg py-1 px-4 bg-red-50 text-base lg:text-sm outline-none  text-red-600 text-opacity-90 placeholder-opacity-70 placeholder-red-600 focus:border-red-200' || 'border-2 border-white focus:border-blue-200  w-full rounded-lg py-1 px-4 bg-slate-50 text-base lg:text-sm  outline-none placeholder-blue-400 text-blue-900 text-opacity-90 placeholder-opacity-70'}`}
                 type={( show === true ? 'password' : 'text')} placeholder={`${passwordError && 'please enter password' || 'Password'}`} /> <div className={( passwordError ? 'absolute top-2 right-4 text-sm text-red-600 opacity-50 z-10' :  'absolute top-2 right-4 text-sm bg-transparent text-blue-400 opacity-50 z-10')}><FaLock/></div>
                </div>
            ...
            <div className=''>
              <span className="text-sm flex gap-2">Already have an account? <Link href={"/login"} className='text-sm bg-gradient-to-r from-sky-600  to-cyan-900 text-transparent bg-clip-text cursor-pointer'>Signin here</Link></span>
            </div>
        </div>
      </div>
  )
